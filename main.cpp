...
void mouseMotion(int x, int y) {
    ...
    int centerX = windowWidth / 2; int centerY = windowHeight / 2;
    if (x == centerX && y == centerY) return;

    float sensitivity = 0.003f;
    yaw += (x - centerX) * sensitivity; pitch -= (y - centerY) * sensitivity;
...
void mouseClick(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN && gameState == 0) {
        ...
        if (x >= 350 && x <= 550 && y >= 230 && y <= 270) {
            gameState = 1; glutSetCursor(GLUT_CURSOR_NONE); glutWarpPointer(windowWidth/2, windowHeight/2);
        } else if (x >= 350 && x <= 550 && y >= 180 && y <= 220) gameState = 2;
        else if (x >= 350 && x <= 550 && y >= 130 && y <= 170) exit(0);
    }
}
...
void keyboardDown(unsigned char key, int x, int y) { 
    ...
    if (gameState == 0) {
        if (key == 13) { gameState = 1; glutSetCursor(GLUT_CURSOR_NONE); glutWarpPointer(windowWidth/2, windowHeight/2); } 
        else if (key == 'h') gameState = 2; else if (key == 27) exit(0);
    } else if (gameState == 1) {
        keys[key] = true; 
        if (key == '=') {
            isTestMap = !isTestMap; 
            if (isTestMap) { ballX = 0.0f; ballY = 5.0f; ballZ = 0.0f; }
            else { ballX = respawnX; ballY = respawnY + 2.0f; ballZ = respawnZ; }
            velX = 0; velZ = 0;
        }
        else if (key == 'b') { gameState = 0; glutSetCursor(GLUT_CURSOR_INHERIT); }
        else if (key == 27) exit(0);
    } else if (gameState == 2) {
        if (key == 'b' || key == 27) gameState = 0;
    }
}
...
void timer(int value) {
    if (gameState == 1) {
        float dirX = sin(yaw); float dirZ = -cos(yaw);
        float rightX = cos(yaw); float rightZ = sin(yaw);
        float inputX = 0.0f; float inputZ = 0.0f;
        bool hasInput = false;

        if (keys['w']) { inputX += dirX; inputZ += dirZ; hasInput = true; }
        if (keys['s']) { inputX -= dirX; inputZ -= dirZ; hasInput = true; }
        if (keys['a']) { inputX -= rightX; inputZ -= rightZ; hasInput = true; }
        if (keys['d']) { inputX += rightX; inputZ += rightZ; hasInput = true; }
        ...
        if (isSprinting) targetMaxSpeed = 0.25f; 

        bool canStand = !checkWallCollision(ballX, ballY, ballZ, 0.05f, 0.05f, 1.2f);

        if (tryingToCrouch) {
            if (isGrounded && !isSliding && currentSpeed > 0.10f && !isBraking) isSliding = true; 
            isCrouching = true; currentHeight = 0.6f; 
        } else {
            if (canStand) {
                isCrouching = false; isSliding = false; currentHeight = 1.2f; 
            } else {
                isCrouching = true; currentHeight = 0.6f;
                if (isGrounded && currentSpeed > 0.10f && !isBraking) isSliding = true;
                else isSliding = false;
            }
        }

        if (isCrouching && !isSliding) targetMaxSpeed = 0.06f; 
        
        if (isSliding) {
            if (currentSpeed < 0.02f) { isSliding = false; velX = 0.0f; velZ = 0.0f; }
            else if (isBraking) { 
                isSliding = false; velX = 0.0f; velZ = 0.0f; 
            } 
        }
        ...
        if (!isGrounded && hittingWallForward && headClear) {
            ...
            if (isLedge) {
                if (isHanging) {
                    if (keys['w']) {
                        speedY = 0.30f; velX = dirX * 0.15f; velZ = dirZ * 0.15f;
                ...
                if (keys['w'] && climbTimer < MAX_CLIMB) {
                    isClimbing = true; isHanging = false; climbTimer++;
                } else {
                    isClimbing = false; isHanging = false;
                }
            }
        } else {
            isHanging = false; isClimbing = false;
            if (isGrounded) climbTimer = 0; 
        }

        if (isHanging && (keys['s'] || keys['a'] || keys['d'] || keys['c'])) isHanging = false;

        float friction = 1.0f; float accel = 0.0f; float currentGravity = gravity;

        if (isHanging) {
            currentGravity = 0.0f; speedY = 0.0f; velX = 0.0f; velZ = 0.0f; 
        } else if (isClimbing) {
            currentGravity = 0.0f; speedY = 0.08f; velX = 0.0f; velZ = 0.0f; 
        } else if (isSliding) {
            friction = 0.985f; accel = 0.0f; targetMaxSpeed = 1.0f; 
        } else if (isGrounded) {
            if (hasInput) { friction = 0.85f; accel = targetMaxSpeed * (1.0f - friction); } 
            else { friction = 0.40f; accel = 0.0f; }
        } else {
            friction = 0.99f; accel = hasInput ? 0.005f : 0.0f; 
        }

        float targetRadX = 0.3f; float targetRadZ = 0.3f;
        if (isSliding && currentSpeed > 0.05f) {
            targetRadX = 0.3f + abs(velX/currentSpeed) * 0.6f; targetRadZ = 0.3f + abs(velZ/currentSpeed) * 0.6f;
        }
        ...
        if (!checkWallCollision(ballX + velX, ballY, ballZ, currentRadiusX, currentRadiusZ, currentHeight)) ballX += velX;
        else velX = 0; 
        if (!checkWallCollision(ballX, ballY, ballZ + velZ, currentRadiusX, currentRadiusZ, currentHeight)) ballZ += velZ;
        else velZ = 0;
        ...
        if (!isTestMap) {
            float cMinX = currChunk.x[18] - (currChunk.sx[18]/2); float cMaxX = currChunk.x[18] + (currChunk.sx[18]/2);
            float cMinZ = currChunk.z[18] - (currChunk.sz[18]/2); float cMaxZ = currChunk.z[18] + (currChunk.sz[18]/2);
...
void display() {	
    ...
    if (gameState == 0) drawMenu(); else if (gameState == 1) { drawGame3D(); drawHUD(); } else if (gameState == 2) drawHelp();
    glutSwapBuffers();
}
...
void Reshape(int w, int h) {
    ...
    gluPerspective(75.0, (GLfloat)w / (GLfloat)h, 0.1, 300.0); glMatrixMode(GL_MODELVIEW); glLoadIdentity(); 
}

void init() {
    glClearColor(0.2, 0.6, 0.8, 1.0); glEnable(GL_DEPTH_TEST); setupLighting(); 
    titleTexture = loadTexture("C:\\Users\\TaiBalap\\Documents\\Praktikum\\GTI\\Tubes\\RandomParkourGame\\sementara.png");
}

int main(int argc, char **argv) {
    glutInit(&argc, argv); glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH); 
    glutInitWindowSize(windowWidth, windowHeight); glutCreateWindow("Apex Movement - Bug Fixed"); 
    
    init(); srand(time(NULL)); for(int i=0; i<256; i++) keys[i] = false; 
    ...
    ballX = currChunk.x[0]; ballY = currChunk.y[0] + 0.25f; ballZ = currChunk.z[0];
    respawnX = ballX; respawnY = ballY; respawnZ = ballZ;
    
    glutDisplayFunc(display); glutReshapeFunc(Reshape); 
    glutKeyboardFunc(keyboardDown); glutKeyboardUpFunc(keyboardUp); glutMouseFunc(mouseClick);
