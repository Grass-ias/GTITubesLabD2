...
GLuint loadTexture(const char* filename) {
    ...
    if (!data) { printf("Gagal load image: %s\n", filename); return 0; }
...
void drawImage(GLuint tex, float x, float y, float w, float h) {
    glEnable(GL_TEXTURE_2D); glBindTexture(GL_TEXTURE_2D, tex); glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    glColor3f(1, 1, 1);
...
void begin2D() {
    glDisable(GL_DEPTH_TEST); glMatrixMode(GL_PROJECTION); glPushMatrix(); glLoadIdentity();
    gluOrtho2D(0, windowWidth, 0, windowHeight); glMatrixMode(GL_MODELVIEW); glPushMatrix(); glLoadIdentity();
}
...
void drawMenu() {
    begin2D(); drawImage(titleTexture, 230, 330, 440, 220); glColor3f(1, 1, 1);
    drawText(400, 250, "START"); drawText(405, 200, "HELP"); drawText(407, 150, "QUIT");
    drawText(275, 80, "ENTER = Start | H = Help | ESC = Quit"); end2D();
}

void drawHelp() {
    begin2D(); glColor3f(1, 1, 1); drawText(420, 520, "Key"); drawText(220, 430, "WASD  = Move | Q = Sprint");
    drawText(220, 390, "MOUSE = Look | C = Crouch/Slide"); drawText(220, 350, "SPACE = Jump | = = Map Test");
    drawText(220, 310, "B     = Back to Menu"); drawText(220, 270, "ESC   = Back / Quit");
    drawText(210, 140, "Press B or ESC to return to menu"); end2D();
}

void drawHUD() {
    begin2D(); glColor3f(1, 1, 1);
    char debugText[100]; float currentSpeed = sqrt(velX*velX + velZ*velZ) * 100.0f; 
    sprintf(debugText, "Speed: %.0f | Map: %s", currentSpeed, isTestMap ? "TEST ROOM" : "INFINITE OBBY");
...
void drawGame3D() {
    ...
    for (int c = 0; c < 3; c++) {
        ...
        for (int i = 0; i < JUMLAH_PLATFORM; i++) {
            if (c == 2 && i == 0) continue; 
            if (c == 0 && i == 18) continue;
            glPushMatrix();
            glTranslatef(chunks[c]->x[i], chunks[c]->y[i], chunks[c]->z[i]); glScalef(chunks[c]->sx[i], 0.5, chunks[c]->sz[i]); 
            if (chunks[c]->type[i] == 0) { if (i == 18) glColor3f(0.0, 0.0, 1.0); else glColor3f(0.0, 1.0, 0.0); } 
            else glColor3f(1.0, 0.5, 0.0);              
            glutSolidCube(1.0); glColor3f(0.0, 0.0, 0.0); glutWireCube(1.0); 
            glPopMatrix();
        }
    }
}
